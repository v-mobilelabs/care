...
} from "@/workflow/chat-api-flow.workflow.js";
import { runNutritionMealPlannerGraph } from "@/workflow/nutrition-meal-planner.workflow.js";
...
/**
 * POST /api/v1/chat
 * Streaming chat endpoint using the Router Agent + subagents pattern.
 *
...
router.post("/api/v1/chat", async (req, res) => {
  ...
  logger.info("[Chat Route] Request received", { uid });
  ...
  try {
    ...
    pipeUIMessageStreamToResponse({
      ...
      consumeSseStream: ({ stream: sseStream }) => {
        consumeStream({
          stream: sseStream,
          onError: (error) =>
            logger.error("[Chat API] consumeStream error:", error),
        });
      },
    });
  } catch (error) {
    ...
    logger.error("[Chat Route] Unexpected error", {
      uid,
      error:
