...
export class LabReportExtractionService {
  constructor(
  ...
  async extract(input: ExtractLabReportInput): Promise<LabReportDto> {
    ...
    if (!file) {
      throw Object.assign(
        new Error(
          `[LabReportExtractionService] File not found — userId=${input.userId} fileId=${input.fileId}`,
        ),
        { code: "FILE_NOT_FOUND" },
      );
    }
...
export const labReportExtractionService = new LabReportExtractionService();
