...
import { AssistantWelcome } from "./_components/assistant-welcome";
import { ContinuityCues } from "./_components/continuity-cues";
import { StarterCards } from "./_components/starter-cards";
import { SessionRecap } from "./_components/session-recap";
...
function buildGcsFilePart(
    file: Pick<FileRecord, "id" | "mimeType" | "storagePath" | "name" | "label">,
): {
    type: "file";
    fileId?: string;
    url: string;
...
export function ChatContent() {
    ...
    const [recapModalOpened, { open: openRecapModal, close: closeRecapModal }] = useDisclosure(false);
    const [continuityModalOpened, { open: openContinuityModal, close: closeContinuityModal }] = useDisclosure(false);
    ...
    const checkAndSend = async (text: string, files?: FileList, existingFiles?: FileRecord[]): Promise<void> => {
        ...
        if (fileParts.length > 0) {
            setPendingAttachments(
                fileParts.map(({ fileId, url, mediaType, fileName, label }) => ({
                    ...(fileId ? { fileId } : {}),
                    url,
                    mediaType,
                    ...(fileName ? { fileName } : {}),
    ...
    useEffect(() => {
        ...
        router.replace(`/user/assistant?${params.toString()}`);
    ...
    const inputBarNode = (
        <InputBar
            ...
            onStopAndNewChat={(text, files) => {
                ...
                router.replace(`/user/assistant?id=${newId}`);
            }}
    ...
    return (
        <Box style={{ position: "relative" }}>
                    ...
                    >
                        ...
                        <Box
                            style={{
                                ...
                                height: 40,
                                pointerEvents: "none",
                                background: "linear-gradient(to bottom, transparent, light-dark(var(--mantine-color-gray-0), var(--mantine-color-dark-9)))",
                            }}
                        />
            ...
            {/* Recap modal */}
            <Modal opened={recapModalOpened} onClose={closeRecapModal} title="Session recap" centered size="lg">
                {hasUserMessages && <SessionRecap messages={messages} />}
            </Modal>

            {/* Continuity modal */}
            <Modal opened={continuityModalOpened} onClose={closeContinuityModal} title="Continuity assessment" centered size="md">
                {hasUserMessages && <ContinuityCues messages={messages} />}
            </Modal>
        </Box >
    );
}
