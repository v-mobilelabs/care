...
export function DietPlansContent() {
    ...
    function handleCreateDietPlan() {
        ...
        startTransition(() => {
            router.push(`/user/assistant?id=${id}&message=${encodeURIComponent("I want to build a diet plan")}`);
        });
    }
            ...
            onConfirm: () => {
                deleteMutation.mutate(id, {
                    ...
                    onError: () =>
                        notifications.show({
                            title: "Delete failed",
                            message: "Could not delete diet plan. Please try again.",
                            color: colors.danger,
                        }),
                });
            },
    ...
    const paginated = plans.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
                        ...
                        <Stack gap="sm">
                            {paginated.map((plan) => (
                                <DietPlanCard
                                    ...
                                    isOptimistic={plan.id.startsWith("__optimistic__")}
                                />
                            ))}
                            {totalPages > 1 && (
