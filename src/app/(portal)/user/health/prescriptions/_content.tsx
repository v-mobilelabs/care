        ...
        Array.from(files).forEach((file) => {
            upload.mutate({ file }, {
                onSuccess: (result) => {
                    ...
                    if (count > 0) router.push(`/user/health/prescriptions/${result.id}`);
                },
                onError: (err) =>
                    notifications.show({
                        title: "Upload failed",
    ...
    function handleExtract(file: PrescriptionRecord) {
        ...
        extract.mutate({ fileId: file.id }, {
            onSuccess: (result) => {
                ...
                if (count > 0) router.push(`/user/health/prescriptions/${file.id}`);
            },
            onError: (err) => {
                setExtractingFileId(null);
                notifications.show({
                            ...
                            <SimpleGrid cols={{ base: 1, xs: 2, sm: 3, lg: 4 }} spacing="md">
                                {paginated.map((rx) => (
                                    <PrescriptionCard
                                        ...
                                        onOpenDetail={() => router.push(`/user/health/prescriptions/${rx.id}`)}
