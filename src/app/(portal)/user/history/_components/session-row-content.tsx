...
function getSessionHint(session: SessionSummary): string {
    if (session.lastMessagePreview) {
        return session.lastMessagePreview;
    }

    if (session.messageCount <= 1) {
        return "Short conversation — open it to continue where you left off.";
    }

    if (session.lastAgentType === "prescription") {
        return "Useful for medication clarifications and refill follow-ups.";
    }

    if (session.lastAgentType === "labReport") {
        return "Useful for reviewing lab findings and follow-up questions.";
    }

    if (session.messageCount <= 4) {
        return `Quick thread with ${session.messageCount} messages — easy to pick back up.`;
    }

    if (session.messageCount <= 12) {
        return `Steady follow-up thread with ${session.messageCount} messages of prior context.`;
    }

    return `Detailed conversation with ${session.messageCount} messages you can reference.`;
}
