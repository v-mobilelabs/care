...
export function useSetMessagesCache(sessionId: string) {
  ...
  return (records: MessageRecord[]) => {
    qc.setQueryData<MessageRecord[]>(chatKeys.messages(sessionId), records);
  };
}
...
export function useUploadBloodTestMutation() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [...chatKeys.bloodTests(), pid] });
    },
  });
}
...
export function useDeleteBloodTestMutation() {
  ...
  return useMutation({
    ...
    onSettled: () => {
      qc.invalidateQueries({ queryKey: btKey });
    },
  });
}
