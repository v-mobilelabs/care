...
export function parseInline(text: string, baseKey = 0): React.ReactNode {
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        if (raw.startsWith("***")) {
            ...
            parts.push(
                <code key={key++} style={{ fontFamily: "monospace", fontSize: "0.85em", padding: "1px 5px", borderRadius: 4, background: "light-dark(var(--mantine-color-gray-2), var(--mantine-color-dark-5))" }}>
                    {raw.slice(1, -1)}
                </code>
            );
        }
