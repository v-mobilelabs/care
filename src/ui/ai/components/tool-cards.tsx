...
import type { AskQuestionInput, QuestionType, StartAssessmentInput } from "@/ui/ai/types";
...
export function QuestionCard({ data, toolCallId, isAnswered, answeredValue, isLoading, onAnswer }: Readonly<QuestionCardProps>) {
    ...
    return (
        <Paper
            ...
            p={{ base: "sm", xs: "md" }}
            my={{ base: "md", xs: "xl" }}
            style={{ background: "light-dark(var(--mantine-color-gray-0), var(--mantine-color-dark-6))" }}
        >
            <Stack gap="sm">
                ...
                <Stack gap={4}>
                    ...
                    {isAnswered && <Badge color="teal" size="sm" variant="light" leftSection={<IconCheck size={10} />} ml={40}>Answered</Badge>}
                </Stack>
