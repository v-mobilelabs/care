...
import { useMic } from "@/ui/chat/hooks/use-mic";
import { useLiveSpeech } from "@/ui/chat/hooks/use-live-speech";
import { LiveOverlay } from "@/ui/chat/components/live-overlay";
import { FilePickerDrawer } from "./file-picker-drawer";
import type { FileRecord } from "@/ui/chat/query";
...
export function InputBar({
    ...
    const { liveMode, livePhase, liveTranscript, liveAIText, closeLive } =
        useLiveSpeech({
            messages,
            status,
            onSendMessage: (text: string) => onSend(text),
        });
    ...
    return (
        <>
                ...
                <Box maw={760} mx="auto">
                    ...
                    <Card
                        ...
                        style={{
                            ...
                            backdropFilter: "none",
                            transition: `border-color ${motionTokens.duration.fast} ${motionTokens.easing.standard}`,
                            ...(pendingFreeText ? { borderColor: "var(--mantine-color-primary-4)" } : {}),
                        }}
                    >
                        <Card.Section px="sm" py="sm">
                                        ...
                                        return (
                                            ...
                                            >
                                                {isImg && file.downloadUrl
                                                    ? <Image src={file.downloadUrl} w={24} h={24} radius="sm" fit="cover" alt="" style={{ flexShrink: 0 }} />
                            ...
                            <Textarea
                                ref={textareaRef}
                                placeholder={pendingFreeText ? "Type your answer…" : hasPendingToolCall ? "Please respond to the question above first…" : "Describe your symptoms…"}
            ...
            <style>{`@keyframes pulse-ring{0%{box-shadow:0 0 0 0 rgba(250,82,82,.55)}100%{box-shadow:0 0 0 7px rgba(250,82,82,0)}}`}</style>
