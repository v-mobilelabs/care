...
async function fetchCachedProfileContext(
  ...
  if (onFetchProfile) {
    const profile = await onFetchProfile(userId).catch((error) => {
      console.warn(
  ...
  const profile = await new GetProfileUseCase().execute({ userId }).catch(() => null);
  return profile && "kind" in profile ? profile : null;
}
...
async function inspectKnownContextNode(
  ...
  if (input.profile === undefined) {
    for (const lookupId of profileLookupIds) {
      ...
      profile = await fetchCachedProfileContext(lookupId, input.onFetchProfile);
