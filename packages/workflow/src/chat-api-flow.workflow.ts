...
export type ChatPersistenceGraphInput = {
  ...
  onRevalidateTag?: (tag: string, type?: any) => void;
};
...
async function finalizePersistenceNode(
  ...
  const revalidate = state.onRevalidateTag;
  if (revalidate) {
    revalidate(CacheTags.usage(state.userId), "seconds");
    revalidate(CacheTags.sessions(state.userId), "seconds");
    revalidate(CacheTags.assessments(state.userId), "minutes");
    revalidate(CacheTags.medications(state.userId), "minutes");
    revalidate(CacheTags.patientSummaries(state.userId), "minutes");
    revalidate(CacheTags.symptomObservations(state.userId), "minutes");
  }
  ...
  try {
    ...
    if (isFirestoreMissingUpdateTargetError(err)) {
      console.warn(
        `[Chat API] Cannot sync agentType: session ${state.sessionId} not found. skipping.`,
      );
    } else {
      console.error(
        `[Chat API] Error sync agentType for session ${state.sessionId}:`,
        err,
      );
    }
  }
  ...
  try {
    await extractAndSaveMemories({
      ...
      onRevalidateTag: state.onRevalidateTag,
    });
  } catch (memErr) {
...
const chatPersistenceGraph = new StateGraph(
  Annotation.Root({
    ...
    onRevalidateTag: Annotation<((tag: string, type?: "seconds" | "minutes" | "layout" | "page") => void) | undefined>(),
  }),
)
...
export function scheduleChatPersistence(args: {
  ...
  const run = args.runInBackground ?? ((fn) => setTimeout(fn, 0));
  run(async () => {
    ...
    await runChatPersistenceWithRetry({
      ...
      onRevalidateTag: args.onRevalidateTag,
    });
  });
}
