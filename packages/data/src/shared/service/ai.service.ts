...
export class AIService {
   ...
   * @param opts.useFast  - Use the fast (Flash) model instead of pro.
   * @param opts.useLite - Use the lite model for classification.
   * @param opts.skipCredit - Skip credit consumption (meta-operations).
   * @param opts.modelConfig - Vertex AI model options (temperature, thinking, safety, etc).
   */
  async extractObject<T extends z.ZodTypeAny>(
    ...
    opts: {
      ...
      useLite?: boolean;
      skipCredit?: boolean;
      /** Vertex AI provider options (temperature, thinking config, safety, etc). */
      modelConfig?: Partial<GoogleLanguageModelOptions>;
    },
  ): Promise<z.infer<T>> {
    ...
    const result = await runWithTransientRetry(() => {
      // Add Vertex AI provider options if specified
      if (opts.modelConfig) {
        return generateText({
          ...baseOpts,
          providerOptions: {
            vertex: opts.modelConfig,
          },
        });
      }
   ...
   * @param opts.userId - Required for credit accounting.
   * @param opts.useFast - Use fast model instead of pro.
   * @param opts.useLite - Use lite model for classification.
   * @param opts.skipCredit - Skip credit consumption.
   * @param opts.modelConfig - Vertex AI provider options.
   */
