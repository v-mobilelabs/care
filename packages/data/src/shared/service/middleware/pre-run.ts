...
export async function fetchMemory(
  ...
  const fetchFn = onFetchMemory ?? (async (id) => {
    const { memoryService } = await import("@/data/memory/service/memory.service");
    return memoryService.formatForPrompt(id);
  });

  return withTimeout(
    fetchFn(profileId).catch((err: unknown) => {
