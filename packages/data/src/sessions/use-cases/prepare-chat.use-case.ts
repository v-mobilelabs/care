...
export type PrepareChatInput = z.infer<typeof PrepareChatSchema> & {
  onFetchProfile?: (profileId: string) => Promise<ProfileDto | null>;
  onFetchMemory?: (profileId: string) => Promise<string | null>;
};
...
async function fetchPreparedProfile(
  ...
  if (onFetchProfile) {
    const profile = await onFetchProfile(profileId).catch(() => null);
  ...
  const profile = await new GetProfileUseCase().execute({ userId: profileId }).catch(() => null);
  return profile && "kind" in profile ? profile : null;
}
...
async function resolveGatewayState(args: {
  ...
  const ragIntentClassifierPromise =
    ...
    const orchestratorArgs = {
      ...
      onFetchProfile: args.onFetchProfile,
    } as const;
...
> {
  static validate(input: unknown): PrepareChatInput {
    const parsed = PrepareChatSchema.parse(input);
    return {
      ...parsed,
      onFetchProfile: (input as any).onFetchProfile,
      onFetchMemory: (input as any).onFetchMemory,
    };
  }
  ...
  protected async run(input: PrepareChatInput): Promise<PrepareChatResult> {
    ...
    const memoryPromise = fetchMemory(profileId, input.onFetchMemory);
    ...
    const [{ lastAgentType, groundingCache }, gatewayMetadata, profile] =
      await Promise.all([
        ...
        fetchPreparedProfile(profileId, input.onFetchProfile),
      ]);
    console.log(
